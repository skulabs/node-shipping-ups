...
function UPS(args) {
  ...
  function buildShipment(data, options) {
    ...
    if(data.saturday_delivery) {
      ...
      shipment.ShipmentServiceOptions['SaturdayDelivery'] = [];
    }

    if (data.international_forms) {
      if (!shipment.ShipmentServiceOptions) {
        shipment.ShipmentServiceOptions = {};
      }

      shipment.ShipmentServiceOptions['InternationalForms'] = data.international_forms;
    }

    if (data.return_label  && data.return_label_type) {
      if (!shipment.ShipmentServiceOptions) {
        shipment.ShipmentServiceOptions = {};
      }

      shipment.ShipmentServiceOptions['ReturnService'] = data.return_label_type;
    }
    ...
    if (data.usps_endorsement) {
      shipment['USPSEndorsement'] = data.usps_endorsement;
    }

    if (data.cost_center) {
      shipment['CostCenter'] = data.cost_center;
    }

    if (data.package_id) {
      shipment['PackageID'] = data.package_id;
    }
  ...
  function buildPaymentInformation(data, options) {
    ...
    if (data.third_party_billing) {
      var payment = {
        'BillThirdParty': {
          'BillThirdPartyShipper': {
            'AccountNumber': data.third_party_billing.account_number,
            'ThirdParty': {
              'Address': {
                'PostalCode': data.third_party_billing.billing_zip,
                'CountryCode': data.third_party_billing.billing_country
              }
            }
          }
        }
